...
export function routeQuery(q: string): RouterResult {
  ...
  if (/master\s*plan|site\s*plan|landscape|site\s*layout|campus|overall\s*plan|where.*amenities|land\s*parcel|how\s*big.*(project|loft|site|land)|total\s*(acres|acre|area)|plot\s*size|\d+\s*acre|total\s*units|density|how\s*many\s*(units|flats|apartments|towers|floors)/.test(ql)) {
    return {
      text: `<p>ASBL Loft sits on <strong>4.92 acres</strong> with <strong>2 towers (G+45, 10 units/floor)</strong> — 894 units total, exclusively 3 BHK. 26-zone landscape arranged around the towers + a 55,000 sqft clubhouse to the north-west.</p>`,
